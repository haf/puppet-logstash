...
#   Example:  filter {   zeromq {     add_field =&gt; [
...
#   %{field} Example:  filter {   zeromq {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/zeromq
#
...
define logstash::filter::zeromq (
  ...
  file { $conffiles:
    ...
    content => "filter {\n zeromq {\n${opt_add_field}${opt_add_tag}${opt_address}${opt_field}${opt_mode}${opt_remove_field}${opt_remove_tag}${opt_sockopt} }\n}\n",
    mode    => '0440',
