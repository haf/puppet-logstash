...
#   Example:  filter {   alter {     add_field =&gt; [
...
#   %{field} Example:  filter {   alter {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/alter
#
...
define logstash::filter::alter (
  ...
  file { $conffiles:
    ...
    content => "filter {\n alter {\n${opt_add_field}${opt_add_tag}${opt_coalesce}${opt_condrewrite}${opt_condrewriteother}${opt_remove_field}${opt_remove_tag} }\n}\n",
    mode    => '0440',
