...
#   Example:  filter {   syslog_pri {     add_field =&gt; [
...
#   %{field} Example:  filter {   syslog_pri {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/syslog_pri
#
...
define logstash::filter::syslog_pri (
  ...
  file { $conffiles:
    ...
    content => "filter {\n syslog_pri {\n${opt_add_field}${opt_add_tag}${opt_facility_labels}${opt_remove_field}${opt_remove_tag}${opt_severity_labels}${opt_syslog_pri_field_name}${opt_use_labels} }\n}\n",
    mode    => '0440',
