...
#   can find them here:
#   https://github.com/logstash/logstash/tree/v1.2.2.dev/patterns. You can
...
#   works by combining text patterns into something that matches your
#   logs.  The syntax for a grok pattern is %{SYNTAX:SEMANTIC}  The SYNTAX
#   is the name of the pattern that will match your text. For example,
#   "3.44" will be matched by the NUMBER pattern and "55.3.244.1" will be
#   matched by the IP pattern. The syntax is how you match.  The SEMANTIC
#   is the identifier you give to the piece of text being matched. For
#   example, "3.44" could be the duration of an event, so you could call
#   it simply 'duration'. Further, a string "55.3.244.1" might identify
#   the 'client' making a request.  Optionally you can add a data type
#   conversion to your grok pattern. By default all semantics are saved as
#   strings. If you wish to convert a semantic's data type, for example
#   change a string to an integer then suffix it with the target data
#   type. For example %{NUMBER:num:int} which converts the 'num' semantic
#   from a string to an integer. Currently the only supported conversions
#   are int and float.  Example  With that idea of a syntax and semantic,
#   we can pull out useful fields from a sample log like this fictional
#   http request log:  55.3.244.1 GET /index.html 15824 0.043   The
#   pattern for this could be:  %{IP:client} %{WORD:method}
#   %{URIPATHPARAM:request} %{NUMBER:bytes} %{NUMBER:duration}   A more
#   realistic example, let's read these logs from a file:  input {   file
#   {     path =&gt; "/var/log/http.log"   } } filter {   grok {     match
...
#   logs have a 'queue id' that is an 10 or 11-character hexadecimal
#   value. I can capture that easily like this:
#   (?&lt;queue_id&gt;[0-9A-F]{10,11})   Alternately, you can create a
#   custom patterns file.  Create a directory called patterns with a file
#   in it called extra (the file name doesn't matter, but name it
#   meaningfully for yourself) In that file, write the pattern you need as
#   the pattern name, a space, then the regexp for that pattern. For
#   example, doing the postfix queue id example as above:  # in
#   ./patterns/postfix  POSTFIX_QUEUEID [0-9A-F]{10,11}   Then use the
#   patterns_dir setting in this plugin to tell logstash where your custom
#   patterns directory is. Here's a full example with a sample log:  Jan
#   1 06:25:43 mailserver14 postfix/cleanup[21403]: BEF25A72965:
#   message-id=&lt;20130101142543.5828399CCAF@mailserver14.example.com&gt;
#   filter {   grok {     patterns_dir =&gt; "./patterns"     match =&gt;
#   [ "message", "%{SYSLOGBASE} %{POSTFIX_QUEUEID:queue_id}:
#   %{GREEDYDATA:syslog_message}" ]   } }   The above will match and
#   result in the following fields:  timestamp: Jan  1 06:25:43 logsource:
#   mailserver14 program: postfix/cleanup pid: 21403 queue_id: BEF25A72965
#   syslog_message:
#   message-id=&lt;20130101142543.5828399CCAF@mailserver14.example.com The
...
#
# [*overwrite*]
#   The fields to overwrite.  This allows you to overwrite a value in a
#   field that already exists.  For example, if you have a syslog line in
#   the 'message' field, you can overwrite the 'message' field with part
#   of the match like so:  filter {   grok {     match =&gt; [
#   "message",       "%{SYSLOGBASE} %{DATA:message}     ]     overwrite
#   =&gt; [ "message" ]   } }    In this case, a line like "May 29
#   16:37:11 sadness logger: hello world"  will be parsed and 'hello
#   world' will overwrite the original message.
#   Value type is array
#   Default value: []
#   This variable is optional
#
...
#   %{field} Example:  filter {   grok {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#
# [*tag_on_failure*]
#   Append values to the 'tags' field when there has been no successful
#   match
#   Value type is array
...
#  http://logstash.net/docs/1.2.2.dev/filters/grok
#
...
define logstash::filter::grok (
  ...
  if ($overwrite != '') {
    validate_array($overwrite)
    $arr_overwrite = join($overwrite, '\', \'')
    $opt_overwrite = "  overwrite => ['${arr_overwrite}']\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n grok {\n${opt_add_field}${opt_add_tag}${opt_break_on_match}${opt_drop_if_match}${opt_keep_empty_captures}${opt_match}${opt_named_captures_only}${opt_overwrite}${opt_patterns_dir}${opt_remove_field}${opt_remove_tag}${opt_singles}${opt_tag_on_failure} }\n}\n",
    mode    => '0440',
