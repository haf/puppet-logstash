...
#   Example:  filter {   translate {     add_field =&gt; [
...
#   %{field} Example:  filter {   translate {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/translate
#
...
define logstash::filter::translate (
  ...
  file { $conffiles:
    ...
    content => "filter {\n translate {\n${opt_add_field}${opt_add_tag}${opt_destination}${opt_dictionary}${opt_dictionary_path}${opt_exact}${opt_fallback}${opt_field}${opt_override}${opt_regex}${opt_remove_field}${opt_remove_tag} }\n}\n",
    mode    => '0440',
    notify  => Service[$services],
