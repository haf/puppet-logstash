...
#   Example:  filter {   mutate {     add_field =&gt; [
...
#   %{field} Example:  filter {   mutate {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/mutate
#
...
define logstash::filter::mutate (
  ...
  file { $conffiles:
    ...
    content => "filter {\n mutate {\n${opt_add_field}${opt_add_tag}${opt_convert}${opt_gsub}${opt_join}${opt_lowercase}${opt_merge}${opt_remove_field}${opt_remove_tag}${opt_rename}${opt_replace}${opt_split}${opt_strip}${opt_update}${opt_uppercase} }\n}\n",
    mode    => '0440',
