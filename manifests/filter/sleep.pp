...
#   Example:  filter {   sleep {     add_field =&gt; [
...
#   %{field} Example:  filter {   sleep {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/sleep
#
...
define logstash::filter::sleep (
  ...
  file { $conffiles:
    ...
    content => "filter {\n sleep {\n${opt_add_field}${opt_add_tag}${opt_every}${opt_remove_field}${opt_remove_tag}${opt_replay}${opt_time} }\n}\n",
    mode    => '0440',
