...
#   Example:  filter {   split {     add_field =&gt; [
...
#   %{field} Example:  filter {   split {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/split
#
...
define logstash::filter::split (
  ...
  file { $conffiles:
    ...
    content => "filter {\n split {\n${opt_add_field}${opt_add_tag}${opt_field}${opt_remove_field}${opt_remove_tag}${opt_terminator} }\n}\n",
    mode    => '0440',
