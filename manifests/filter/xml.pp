...
#   %{field} Example:  filter {   xml {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#   xml {     source =&gt; "message"   } }   The above would parse the xml
...
#  http://logstash.net/docs/1.2.2.dev/filters/xml
#
...
define logstash::filter::xml (
  ...
  file { $conffiles:
    ...
    content => "filter {\n xml {\n${opt_add_field}${opt_add_tag}${opt_remove_field}${opt_remove_tag}${opt_source}${opt_store_xml}${opt_target}${opt_xpath} }\n}\n",
    mode    => '0440',
