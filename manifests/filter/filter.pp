...
#   Example:  filter {   filter {     add_field =&gt; [
...
#   syntax. Example:  filter {   filter {     add_tag =&gt; [
...
#   %{field} Example:  filter {   filter {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#   Tags can be dynamic and include parts of the event using the %{field}
#   syntax. Example:  filter {   filter {     remove_tag =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/filter
#
...
define logstash::filter::filter (
  ...
  if $logstash::multi_instance == true {
    ...
    $conffiles    = suffix($confdirstart, "/config/filter_${order}_filter_${name}")
    $services     = prefix($instances, $logstash::params::service_base_name)
    $filesdir     = "${logstash::configdir}/files/filter/filter/${name}"

  } else {

    $conffiles = "${logstash::configdir}/conf.d/filter_${order}_filter_${name}"
