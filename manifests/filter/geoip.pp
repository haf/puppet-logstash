...
#   Example:  filter {   geoip {     add_field =&gt; [
...
#   database, the following are available: city_name, continent_code,
#   country_code2, country_code3, country_name, dma_code, ip, latitude,
...
#   %{field} Example:  filter {   geoip {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#
# [*target*]
#   Specify into what field you want the geoip data. This can be useful
#   for example if you have a src_ip and dst_ip and want information of
#   both IP's
#   Value type is string
#   Default value: "geoip"
#   This variable is optional
...
#  http://logstash.net/docs/1.2.2.dev/filters/geoip
#
...
define logstash::filter::geoip (
  ...
  file { $conffiles:
    ...
    content => "filter {\n geoip {\n${opt_add_field}${opt_add_tag}${opt_database}${opt_fields}${opt_remove_field}${opt_remove_tag}${opt_source}${opt_target} }\n}\n",
    mode    => '0440',
    notify  => Service[$services],
