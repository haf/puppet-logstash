...
#   X is the field number, starting from 1).
#   Value type is array
#   Default value: []
#   This variable is optional
#
...
#   %{field} Example:  filter {   csv {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#
# [*target*]
#   Define target for placing the data Defaults to writing to the root of
#   the event.
#   Value type is string
#   Default value: None
#   This variable is optional
...
#  http://logstash.net/docs/1.2.2.dev/filters/csv
#
...
define logstash::filter::csv (
  ...
  file { $conffiles:
    ...
    content => "filter {\n csv {\n${opt_add_field}${opt_add_tag}${opt_columns}${opt_remove_field}${opt_remove_tag}${opt_separator}${opt_source}${opt_target} }\n}\n",
    mode    => '0440',
