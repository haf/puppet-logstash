...
#   Example:  filter {   environment {     add_field =&gt; [
...
#   %{field} Example:  filter {   environment {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/environment
#
...
define logstash::filter::environment (
  ...
  file { $conffiles:
    ...
    content => "filter {\n environment {\n${opt_add_field}${opt_add_field_from_env}${opt_add_tag}${opt_remove_field}${opt_remove_tag} }\n}\n",
    mode    => '0440',
