...
#   Example:  filter {   multiline {     add_field =&gt; [
...
#   %{field} Example:  filter {   multiline {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/multiline
#
...
define logstash::filter::multiline (
  ...
  file { $conffiles:
    ...
    content => "filter {\n multiline {\n${opt_add_field}${opt_add_tag}${opt_negate}${opt_pattern}${opt_patterns_dir}${opt_remove_field}${opt_remove_tag}${opt_stream_identity}${opt_what} }\n}\n",
    mode    => '0440',
