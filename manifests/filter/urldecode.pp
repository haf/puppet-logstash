...
#   Example:  filter {   urldecode {     add_field =&gt; [
...
#   %{field} Example:  filter {   urldecode {     remove_field =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2.dev/filters/urldecode
#
...
define logstash::filter::urldecode (
  ...
  file { $conffiles:
    ...
    content => "filter {\n urldecode {\n${opt_add_field}${opt_add_tag}${opt_all_fields}${opt_field}${opt_remove_field}${opt_remove_tag} }\n}\n",
    mode    => '0440',
