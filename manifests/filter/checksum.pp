...
#   Example:  filter {   checksum {     add_field =&gt; [
...
#  http://logstash.net/docs/1.2.2.dev/filters/checksum
#
...
define logstash::filter::checksum (
  ...
  if ($algorithm != '') {
    if ! ($algorithm in ['md5', 'sha', 'sha1', 'sha256', 'sha384']) {
  ...
  file { $conffiles:
    ...
    content => "filter {\n checksum {\n${opt_add_field}${opt_add_tag}${opt_algorithm}${opt_keys}${opt_remove_field}${opt_remove_tag} }\n}\n",
    mode    => '0440',
