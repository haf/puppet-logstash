...
#   Event Log Name
#   Value type is array
#   Default value: ["Application", "Security", "System"]
#   This variable is optional
#
# [*tags*]
...
#  http://logstash.net/docs/1.2.2.dev/inputs/eventlog
#
...
define logstash::input::eventlog (
  ...
  if ($logfile != '') {
    validate_array($logfile)
    $arr_logfile = join($logfile, '\', \'')
    $opt_logfile = "  logfile => ['${arr_logfile}']\n"
  }
  ...
  file { $conffiles:
    ...
    content => "input {\n eventlog {\n${opt_add_field}${opt_codec}${opt_debug}${opt_logfile}${opt_tags}${opt_type} }\n}\n",
    mode    => '0440',
