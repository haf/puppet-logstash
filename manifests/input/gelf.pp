...
#   following gelf fields to logstash equivalents:  event["message"]
#   becomes fullmessage if no fullmessage, use event["message"] becomes
#   shortmessage if no shortmessage, event["message"] is the raw json
#   input
#   Value type is boolean
#   Default value: true
#   This variable is optional
...
#  http://logstash.net/docs/1.2.2.dev/inputs/gelf
#
...
define logstash::input::gelf (
  ...
  file { $conffiles:
    ...
    content => "input {\n gelf {\n${opt_add_field}${opt_codec}${opt_debug}${opt_host}${opt_port}${opt_remap}${opt_tags}${opt_type} }\n}\n",
    mode    => '0440',
