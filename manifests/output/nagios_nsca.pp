...
#   Value type is string
#   Default value: "%{@timestamp} %{host}: %{message}"
#   This variable is optional
#
# [*nagios_host*]
...
#  http://logstash.net/docs/1.2.2.dev/outputs/nagios_nsca
#
...
define logstash::output::nagios_nsca (
  ...
  file { $conffiles:
    ...
    content => "output {\n nagios_nsca {\n${opt_codec}${opt_host}${opt_message_format}${opt_nagios_host}${opt_nagios_service}${opt_nagios_status}${opt_port}${opt_send_nsca_bin}${opt_send_nsca_config} }\n}\n",
    mode    => '0440',
    notify  => Service[$services],
