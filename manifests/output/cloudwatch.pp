...
#
# [*timeframe*]
#   Constants aggregate_key members Units How often to send data to
#   CloudWatch  This does not affect the event timestamps, events will
#   always have their actual timestamp (to-the-minute) sent to CloudWatch.
#   We only call the API if there is data to send.  See the Rufus
#   Scheduler docs for an explanation of allowed values
#   Value type is string
#   Default value: "1m"
#   This variable is optional
#
...
#  http://logstash.net/docs/1.2.2.dev/outputs/cloudwatch
#
...
define logstash::output::cloudwatch (
  ...
  file { $conffiles:
    ...
    content => "output {\n cloudwatch {\n${opt_access_key_id}${opt_aws_credentials_file}${opt_codec}${opt_dimensions}${opt_field_dimensions}${opt_field_metricname}${opt_field_namespace}${opt_field_unit}${opt_field_value}${opt_metricname}${opt_namespace}${opt_queue_size}${opt_region}${opt_secret_access_key}${opt_timeframe}${opt_unit}${opt_use_ssl}${opt_value} }\n}\n",
    mode    => '0440',
